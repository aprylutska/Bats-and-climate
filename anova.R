...
min(subset_df_clim$Ra.num, na.rm = TRUE) <- NULL   #видалення порожніх комірок та аутлаєра(помилкове значення) 
...
ggplot(data = subset_df_clim, aes(day, Ra.num, colour = year)) +
  geom_point()

mod <- lm(Ra.num ~ year + day + year:day, data=subset_df_clim)
