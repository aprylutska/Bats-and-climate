...
Bats were captured during the month of July in the years 2008, 2011, 2014, and 2019 as part of a bat monitoring program of the National Nature Park “Homilshansky lisy”, located in the Kharkiv Oblast, Ukraine. The capture methodology involved the use of nylon mist nets (12 meters in length and 3 meters in height, with a mesh size of 15 mm). Nets were consistently deployed at the same locations each year. A total of 18 capture nights were conducted annually, with mist nets set from sunset until sunrise. Body weight measurements, rounded to the nearest 0.1 gram, and forearm length measurements, rounded to the nearest 0.1 millimeter, were obtained within 8-10 hours following the capture events.

Data collected during the capture process included species identification, sex and age determination. Age classification was primarily based on the degree of ossification observed in the joints of the finger phalanx of the wing, the size and morphology of nipples for female bats, and for Nyctalus noctula specifically, the wear pattern of canine teeth. All captured bats underwent biometric assessments before being promptly released back into their natural habitat. All procedures involved in the capture, handling, and transportation of bats were conducted with strict adherence to ethical standards and principles of animal welfare and conservation, as outlined by Gannon (2007).

Weather Data Acquisition and Analysis:
Weather data, encompassing precipitation (Prcp), minimum temperature (Tmin), and maximum temperature (Tmax), for the study area were obtained using the easyclimate R package. Additional weather variables, including diurnal temperature range, mean daily temperature, positive daily temperature, and Growing Degree Units (the sum of mean daily temperatures above 0°C from the commencement of the year), were computed for further analysis.
...
Whether weather conditions during the periods of pregnancy and lactation influence the growth of young bats? It was hypothesized that years characterized by warmer and drier conditions during spring would correspond to larger sizes in subadult bats.
...
Відбираємо лише один вид та тільки молодих - вечірниці
```{r}
...
```{r} 
df1 <- subset(df,Year == '2008')

mod <- lm(Ra ~ day, data=df1)
summary (mod)                                            
plot(mod)
ggplot(data = df1, aes(x = day, y = Ra)) +
  geom_point() +
  geom_smooth(method = "lm") 

ggplot(data=df1, aes (x = Ra)) +
  geom_histogram()
```


```{r} 
df2 <- subset(df,Year == '2011')

mod <- lm(Ra ~ day, data = df2)
summary (mod)                                            
plot(mod)
ggplot(data = df2, aes(x = day, y = Ra)) +
  geom_point() +
  geom_smooth(method = "lm") 

ggplot(data=df2, aes (x = Ra)) +
  geom_histogram()
```


```{r} 
df3 <- subset(df, Year == '2014'
              )

mod <- lm(Ra ~ day, data=df3)
summary (mod)                                            
plot(mod)
ggplot(data = df3, aes(x = day, y = Ra)) +
  geom_point() +
  geom_smooth(method = "lm") 

ggplot(data=df3, aes (x = Ra)) +
  geom_histogram()
```
...
```{r} 
df4 <- subset(df, Year == '2019'
              )

mod <- lm(Ra ~ day, data=df4)
summary (mod)                                            
plot(mod)
ggplot(data = df4, aes(x = day, y = Ra)) +
  geom_point() +
  geom_smooth(method = "lm") 

ggplot(data=df4, aes (x = Ra)) +
  geom_histogram()
```

##fit the lm model to Ra_log10 index

```{r} 
df4 <- subset(df,Year == '2019')

mod <- lm(RA_sqDOY ~ day, data=df4)
summary (mod)                                            
plot(mod)
ggplot(data = df4, aes(x = day, y = RA_sqDOY)) +
  geom_point() +
  geom_smooth(method = "lm") 
...
fit the model to Ra_sgDsJ1

```{r} 
df <- subset(df,Year == '2008')

mod <- lm(Ra ~ day, data=nnoc_df)
summary (mod)                                            
plot(mod)
ggplot(data = nnoc_df, aes(x = day, y = Ra)) +
  geom_point() +
  geom_smooth(method = "lm") 

ggplot(data=nnoc_df, aes (x = Ra)) +
  geom_histogram()
```
